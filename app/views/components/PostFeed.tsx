...
export const NewPostsNotification = (
    ...
    return (
        <div
            ...
            hx-get={`/posts?page=1${list ? `&list=${list.hashId}` : ""}`}
            hx-swap="outerHTML"
            hx-target="#posts"
        >
            {display && (
...
export const PostFeed = (
    ...
    return (
        <div
            id="posts"
            hx-ext="ws"
            ws-connect={list?.hashId ? `/lists/${list?.hashId}/ws` : null}
            class={`w-full max-w-5xl mx-auto flex flex-col gap-4 ${
                className || ""
            }`}
        >
